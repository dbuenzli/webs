...
(** Kinds of URL requests and their formatters.
    ...
    A {{!Webs_kurl.type-kind}kind} value represents a subset of
    {{!Webs_kurl.type-bare}bare URL requests} by values of a custom request
    OCaml type. Kinds are then used:

    {ul
    {- For request decoding. Kinds are tupled with a
       {{!Webs_kurl.type-service}service} function and
       {{!Webs_kurl.val-bind}bound} by a
       constant HTTP path in a {{!Webs_kurl.type-tree}service tree}. Given a
    ...
    {- For response encoding. From the service tree an {{!Webs_kurl.type-fmt}URL
    ...
    {b TODO.}
    {ul
    {- Bare.equal and Webs_kurl.t equal.}
    ...
    {- The Webs_kurl name doesn't really reflect that this
    ...
    {- Better decoding combinators to counter the
       non-dryness/non-tuply/uglyness/granulariy of {!Webs_kurl.type-kind}}} *)
...
(** URL request formatters.
    ...
    {b TODO}
    {ul
    {- Move out of [Webs_kurl] as [Fmt_url] or [Urlf] or
       [Urlfmt] or [Url_fmt] ?}} *)
module Fmt : sig

  type kurl = t
  (** See {!Webs_kurl.type-t}. *)
  ...
  (** [empty ~disable_rel ~use_exts ~scheme ~host ~root ()] is
      ...
      {[
        let url uf path = Webs_kurl.Url.url uf Webs_kurl.(V (any, Bare.v `GET path))
      ]}
  *)
  ...
  (** [bind p k uf] is [uf] with URL requests of kind [k] bound to
      {{!Webs_kurl.wf_paths}well-formed} path [p].

      Raises [Invalid_argument] if [k] is already bound in [uf] or
      if [p] not {{!Webs_kurl.wf_paths}well-formed}. *)
...
(** {1:path Path shenanigans}
    ...
    To lessen the mess, in [Webs_kurl] you can only bind to well-formed
